...
def get_static_image(longitude: float, latitude: float, zoom: float, width: int, height: int):
    url = (
    ...
    if response.status_code != 200:
        return HTTPException(status_code=response.status_code, detail="Error fetching MapBox image")

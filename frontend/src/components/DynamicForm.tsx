...
interface Metadata {
  ...
  relationships?: Array<{ relation: string; related_model: string; foreign_keys: string[] }>;
}
...
const DynamicForm: React.FC<DynamicFormProps> = ({ metadata, onSubmit }) => {
  ...
  const relatedTable = relevantRelationship ? `${relevantRelationship.relation}s` : undefined;
  ...
  const { data: relatedData, isLoading: relatedLoading } = useGetDataQuery(
    relatedTable ? { tablename: relatedTable, limit: 100, offset: 0 } : { tablename: "", limit: 0, offset: 0 },
    ...
    <StyledForm onSubmit={handleSubmit(onSubmit)}>
        ...
        .map((field) => {
          ...
          return (
            <StyledInput
              ...
              register={register}
              required={true}
              errorMessage={errors[field.name] ? "Это поле обязательно для заполнения" : undefined}
            />
          );
        })}
      ...
      {relatedTable && foreignKey && (
        ...
        <div style={{ marginTop: "16px" }}>
          <label htmlFor={foreignKey} style={{ display: "block", fontWeight: "bold", marginBottom: "4px" }}>
            Выберите район
          </label>
          <select
            id={foreignKey}
          ...
          {errors[foreignKey] && (
            <span style={{ color: "red", fontSize: "12px" }}>Необходимо выбрать район</span>
          )}
        </div>
      )}
      ...
      <button
        type="submit"
        style={{ marginTop: "24px", padding: "10px 20px", backgroundColor: "#4CAF50", color: "white", border: "none", borderRadius: "4px", cursor: "pointer" }}
      >
        Отправить
      </button>
    </StyledForm>
